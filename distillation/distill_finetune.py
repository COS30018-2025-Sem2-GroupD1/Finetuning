        ...
        with opener(p, "rt", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    # likely a partial/truncated last line — stop cleanly
                    break
                n += 1
                if max_lines is not None and n >= max_lines:
                    break
    except (OSError, EOFError):
        # Truncated gz or CRC mismatch at the end — keep whatever we parsed.
        pass
    return rows
...
def load_soft_map(soft_file: Optional[str], max_lines: Optional[int] = None) -> Dict[str, Any]:
    if not soft_file:
        return {}
    mp: Dict[str, Any] = {}
    rows = read_jsonl_any(soft_file, max_lines=max_lines)
    for rec in rows:
        rid = str(rec.get("id"))
        if rid:
            mp[rid] = rec
...
def _soft_coverage(rows: List[Dict[str, Any]], mp: Dict[str, Any]) -> int:
    return sum(1 for r in rows if str(r.get("id")) in mp)
...
def kd_loss_from_subset_logits(
    ...
    for b in range(B):
        ...
        for j in range(max_j):
            ...
            # Filter OOV ids so we don't index logits out of bounds
            mask = (ids_np >= 0) & (ids_np < V)
            if not mask.any():
                continue
            ids_f = ids_np[mask]
            probs_f = probs_np[mask]
            # Re-normalize teacher probs after filtering
            s = probs_f.sum()
            if s <= 0:
                continue
            probs_f = probs_f / s

            ids_t = torch.tensor(ids_f.tolist(), dtype=torch.long, device=device)
            t_probs = torch.tensor(probs_f.tolist(), dtype=torch.float32, device=device)
...
def main():
    ...
    ap.add_argument("--soft-max-lines", type=int, default=None,
                help="Only read this many lines from --soft-file (useful for truncated .gz)")
    ...
    soft_map_all = load_soft_map(args.soft_file, args.soft_max_lines) if args.soft_file else {}

    if soft_map_all:
        cov_tr = _soft_coverage(train_rows, soft_map_all)
        cov_va = _soft_coverage(val_rows,   soft_map_all)
        cov_te = _soft_coverage(test_rows,  soft_map_all)
        logger.info(f"Soft-label coverage — train: {cov_tr}/{len(train_rows)}  "
                    f"val: {cov_va}/{len(val_rows)}  test: {cov_te}/{len(test_rows)}")

        # Tokenizer-compat sanity (teacher vs student vocab)
        vocab_size = int(getattr(tok, "vocab_size", len(getattr(tok, "get_vocab", lambda: {})())))
        total_steps = 0
        ok_steps = 0
        for rec in soft_map_all.values():
            for st in (rec.get("steps") or []):
                ids = st.get("topk_ids") or []
                if not ids:
                    continue
                total_steps += 1
                # consider a step "ok" if all ids fall inside student's vocab
                if max(ids) < vocab_size and (st.get("chosen_id") is None or st["chosen_id"] < vocab_size):
                    ok_steps += 1
        compat_rate = (ok_steps / total_steps) if total_steps else 0.0
        logger.info(f"Soft-label tokenizer-compat — steps ok: {ok_steps}/{total_steps} ({compat_rate:.4f}), student_vocab={vocab_size}")
    else:
        compat_rate = 0.0
    
    # KD default OFF unless user forces it (since teacher != student tokenizer in your setup)
    use_kd = bool(args.use_soft_labels and (args.force_kd or (compat_rate >= 0.99)))
    if args.use_soft_labels and not args.force_kd and compat_rate < 0.99:
        logger.warning("Soft labels appear incompatible with the student tokenizer (compat_rate<0.99). "
                       "Disabling KD. Use --force-kd to override (we will filter OOV ids).")
    ...
    if _has_field(TrainingArguments, "evaluation_strategy"):
        ...
        if _has_field(TrainingArguments, "greater_is_better"):
         ...
         # encourage explicit save strategy too
            if _has_field(TrainingArguments, "save_strategy"):
    ...
    # Log the key early-stopping fields so you can see them in the log
    logger.info(f"ES wiring — eval_enabled={eval_enabled}, "
            f"load_best_model_at_end={getattr(train_args,'load_best_model_at_end',None)}, "
            f"metric_for_best_model={getattr(train_args,'metric_for_best_model',None)}, "
            f"greater_is_better={getattr(train_args,'greater_is_better',None)}, "
            f"evaluation_strategy={getattr(train_args,'evaluation_strategy',None)}, "
            f"save_strategy={getattr(train_args,'save_strategy',None)}")
    ...
    callbacks = []
    if eval_enabled:
        # make sure the required args are set whenever we add EarlyStopping
        if _has_field(TrainingArguments, "load_best_model_at_end"):
            ta_kwargs["load_best_model_at_end"] = True
        if _has_field(TrainingArguments, "metric_for_best_model"):
            ta_kwargs["metric_for_best_model"] = "eval_loss"
        if _has_field(TrainingArguments, "greater_is_better"):
            ta_kwargs["greater_is_better"] = False
        # encourage explicit step strategies
        if _has_field(TrainingArguments, "save_strategy"):
            ta_kwargs["save_strategy"] = "steps"
        if _has_field(TrainingArguments, "evaluation_strategy"):
            ta_kwargs["evaluation_strategy"] = "steps"
